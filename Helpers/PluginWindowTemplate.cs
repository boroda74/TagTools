...
namespace MusicBeePlugin
{
    public partial class PluginWindowTemplate : Form
    {
        ...
        internal ToolTip getFormToolTip(string toolTipName)
        {
            Type type = this.GetType();
            FieldInfo fieldInfo = type.GetField(toolTipName, BindingFlags.NonPublic | BindingFlags.Instance);
            return fieldInfo?.GetValue(this) as ToolTip;
        }
        ...
        internal void listBox_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            if (e.Index == -1)
                e.Graphics.DrawString(listBox.Text, e.Font, new SolidBrush(textColor), new RectangleF(textPoint, textSize));
            else
        ...
        internal void button_GotFocus(object sender, EventArgs e)
        {
            ...
            else if (acceptButton != button) //Not accept button clicked
            {
                ...
                else if (acceptButton != null && acceptButton.IsEnabled() && !artificiallyFocusedAcceptButton) //It's non-defaultable button & accept button enabled
                {
                    ...
                    acceptButton.Focus(); //Last selected control will be re-selected later (see code above)
                }
        ...
        internal protected Color getButtonBorderColor(bool mouseOver, bool enabled, bool acceptButton)
        {
            ...
            else if (enabled && mouseOver && ButtonMouseOverBackColor != InputPanelBackColor)
        ...
        internal protected void button_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(text))
            {
                ...
                if (useSkinColors && button.FlatStyle == FlatStyle.Flat) //button.FlatStyle == FlatStyle.Flat: not using native Windows buttons, see skinControl()
                {
        ...
        internal protected void Enable(bool state, Control preservedControl1, Control preservedControl2)
        {
            ...
            {
                if (control == preservedControl1 || control == preservedControl2)
                    continue;
                ...
                else if (control.Parent is NumericUpDown)
                {
                    //Let's ignore...
                }
                else if (!control.HasChildren || control is CustomComboBox || control is NumericUpDown)
                {
                    EnableControl(control, state);
                }
            }
        ...
        internal virtual void preMoveScaleControl(Control control)
        {
            ...
            if (control is Button)
            {
                ...
                //Let's change auto-scaled button height to auto-scaled text box height
                if (ButtonHeightDpiFontScaling != 1 || TextBoxHeightDpiFontScaling != 1) //-V3024
                {
                    float nativeButtonScaling = 1;
                    if (!useSkinColors)
                        nativeButtonScaling = (DpiScaling - 1) * 0.32f + 1;

                    int initialHeight = control.Height;
                    control.Height = (int)Math.Round(nativeButtonScaling * control.Height * TextBoxHeightDpiFontScaling / ButtonHeightDpiFontScaling);
        ...
        internal protected void dgvCustomHScrollBar_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (maximum < largeChange)
                (_, maximum, _, largeChange) = GetDataGridViewScrollBarMetrics(dataGridView, sender as CustomHScrollBar);
        ...
        internal protected void dgvCustomVScrollBar_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (maximum < largeChange)
                (_, maximum, _, largeChange) = GetDataGridViewScrollBarMetrics(dataGridView, sender as CustomVScrollBar);
        ...
        internal static (int, int, int, int) GetListBoxScrollBarMetrics(Control scrolledControl, Control refScrollBar)
        {
        ...
        //Returns Minimum, Maximum, SmallChange, LargeChange
        internal static (int, int, int, int) GetTextBoxScrollBarMetrics(Control scrolledControl, Control refScrollBar)
        {
            ...
            if (refScrollBar is CustomVScrollBar && totalLines > 0)
                return (0, totalLines, 1, visibleLines);
            else if (refScrollBar is CustomVScrollBar)
                return (0, 1, 1, 1);
        ...
        private static (CustomHScrollBar, int, CustomVScrollBar, int, bool) UpdateDgvCustomScrollBarsInternal(DataGridView dataGridView)
        {
            ...
            if (customHScrollBar != null)
            {
                (_, int maximum, int smallChange, int largeChange) = GetDataGridViewScrollBarMetrics(dataGridView, customHScrollBar);

                if (customHScrollBar.Maximum != maximum || customHScrollBar.LargeChange != largeChange || customHScrollBar.SmallChange != smallChange)
                {
                    customScrollBarsMetricsChanged = true;

                    customHScrollBar.Minimum = 0;
                    customHScrollBar.Maximum = maximum;
                    customHScrollBar.SmallChange = smallChange;
            ...
            if (customVScrollBar != null)
            {
                (_, int maximum, int smallChange, int largeChange) = GetDataGridViewScrollBarMetrics(dataGridView, customVScrollBar);

                if (customVScrollBar.Maximum != maximum || customVScrollBar.LargeChange != largeChange || customVScrollBar.SmallChange != smallChange)
                {
                    customScrollBarsMetricsChanged = true;

                    customVScrollBar.Minimum = 0;
                    customVScrollBar.Maximum = maximum;
                    customVScrollBar.SmallChange = smallChange;
        ...
        internal static void UpdateCustomScrollBars(Control scrolledControl, int columnOrItemCount = -1, int rowCount = -1)
        {
            ...
            else if (customHScrollBar != null && (columnOrItemCount == -1 || customScrollBarsMetricsChanged))
            {
            ...
            else if (customVScrollBar != null && (rowCount != -1 || customScrollBarsMetricsChanged))
            {
        ...
        //Returns Minimum, Maximum, SmallChange, LargeChange
        internal static (int, int, int, int) GetDataGridViewScrollBarMetrics(Control scrolledControl, Control customScrollBar)
        {
            ...
            if (customScrollBar is CustomHScrollBar hScrollBar)
            {
                ...
                {
                    int visibleColumnCount = dataGridView.Columns.GetColumnCount(DataGridViewElementStates.Visible);
                    if (visibleColumnCount > 0)
                        smallChange = dataGridView.Columns.GetColumnsWidth(DataGridViewElementStates.Visible) / visibleColumnCount;

                    if (dataGridView.DisplayedColumnCount(false) < dataGridView.Columns.GetColumnCount(DataGridViewElementStates.Visible))
                ...
                return (0, maximum, smallChange / 2, largeChange);
            }
            else if (customScrollBar is CustomVScrollBar vScrollBar)
            {
                ...
                {
                    int visibleRowCount = dataGridView.Rows.GetRowCount(DataGridViewElementStates.Visible);
                    if (visibleRowCount > 0)
                        smallChange = dataGridView.Rows.GetRowsHeight(DataGridViewElementStates.Visible) / visibleRowCount;
        ...
        internal void autoSizeTableRows(DataGridView dataGridView, int adjustedColumnIndex)
        {
            if (dataGridView.RowCount > 0)
            {
            ...
            bool itsLastRowRange, bool selectLastRow, bool resetBindingsMetadataChanged = false)
        {
            if (rowsCount > 0)
            {
                ...
                {
                    source.ResetBindings(resetBindingsMetadataChanged);

                    if (selectLastRow && SavedSettings.scrollPreviewToEnd)
                        dataGridView.CurrentCell = dataGridView[0, dataGridView.RowCount - 1];
                    else if (!SavedSettings.scrollPreviewToEnd)
                        dataGridView.CurrentCell = dataGridView[0, 0];

                    if (form.useSkinColors)
                        UpdateCustomScrollBars(dataGridView, 1, rowsCount);
        ...
        internal CustomComboBox getKeyEventCustomComboBox(object sender)
        {
            ...
            else if (sender is ListBox listBox) //It's opened custom combo box list
            {
                customComboBox = listBox.Tag as CustomComboBox;
            }
            else if (lastSelectedControl?.Parent is CustomComboBox)
            {
                customComboBox = lastSelectedControl.Parent as CustomComboBox;
            }
            else if (sender is Control control)
            {
                var focusedControl = FindFocusedControl(control);

                if (focusedControl == null || !(focusedControl.Parent is CustomComboBox customComboBox2))
                    return null;

                customComboBox = customComboBox2;
            }
            else
            {
                return null;
        ...
        internal void CustomComboBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (customComboBox == null || customComboBox.StandardKeyPressHandling)
                return;
            ...
            {
                foreach (object item in customComboBox.Items)
                {
                    string itemName = item == null ? string.Empty : item.ToString();

                    if (itemName.Length > searchedIndex && itemName[searchedIndex].ToString().ToLower() == searchedLetter)
                    {
                        customComboBox.SelectedItem = item;
                        e.Handled = true;
                        return;
                    }
                }
        ...
        internal void CustomComboBox_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            var standardKeyDownHandling = customComboBox.StandardKeyDownHandling;
            if (standardKeyDownHandling != true)
            {
                if (e.KeyCode == Keys.Space && customComboBox.SelectedIndex > -1)
                {
                    e.SuppressKeyPress = true;
                    customComboBox.SelectCurrentDropDownItem();
                }
                else if (e.KeyCode == Keys.Right && standardKeyDownHandling == false && customComboBox.Items.Count > 0)
                {
                    if (customComboBox.SelectedIndex == -1)
                        customComboBox.SelectedIndex = 0;

                    e.SuppressKeyPress = true;
                    customComboBox.SelectCurrentDropDownItem();

                    customComboBox.SelectionStart = 0;
                    customComboBox.SelectionLength = customComboBox.Text.Length;
                }
            }
            ...
            if (e.KeyCode == Keys.Escape)
            {
                ...
                customComboBox.SelectedItem = null;

                if (customComboBox.DropDownStyle == ComboBoxStyle.DropDown)
                    customComboBox.Text = string.Empty;

                customComboBox.DroppedDown = false;
            }
            ...
            {
                e.SuppressKeyPress = customComboBox.OpenCustomDropDown();
            }
        ...
        //It's for handling key up/key down events.
        internal void ListBox_KeyDown(object sender, KeyEventArgs e)
        {
            var listBox = sender as ListBox;
            ...
            else if (e.KeyCode == Keys.PageUp && listBox.SelectedIndex > 19)
            {
                e.SuppressKeyPress = true;
                listBox.SelectedIndex -= 20;
            }
            else if (e.KeyCode == Keys.PageUp)
            {
                e.SuppressKeyPress = true;
            ...
            else if (e.KeyCode == Keys.Down && listBox.SelectedIndex < listBox.Items.Count - 1)
            {
                e.SuppressKeyPress = true;
                listBox.SelectedIndex++;
            }
            else if (e.KeyCode == Keys.PageDown && listBox.SelectedIndex < listBox.Items.Count - 21)
            {
                e.SuppressKeyPress = true;
                listBox.SelectedIndex += 20;
            }
        ...
        internal void setSkinnedControlColors(Control control, bool? enable) //---
        {
            ...
            if (useSkinColors)
            {
                if (control is Button button && (!UseNativeButtonPaint || button.Parent is CustomComboBox)) //button != null always if it is Button
                {
                    var acceptButton = AcceptButton as Button;
                ...
                {
                    customComboBox.SetTextBoxColors(enable);
                }
                else if (control is ComboBox comboBox && comboBox.DropDownStyle == ComboBoxStyle.Simple) //----- Make owner drawing selected items !!!
                {
                    if ((control.IsEnabled() && enable == null) || enable == true)
                    {
                ...
                {
                    //Colors are set by CustomComboBox
                }
        ...
        internal void skinControl(Control control, int borderWidth = -1)
        {
            ...
            if (!useSkinColors)
            {
                ...
                else if (control is ComboBox comboBox)
                {
                    if (comboBox.DropDownStyle == ComboBoxStyle.DropDown || comboBox.DropDownStyle == ComboBoxStyle.DropDownList)
                    {
                        ...
                        customComboBox.comboBox.KeyDown += CustomComboBox_KeyDown;
                    }
                    else
                    {
            ...
            else if (control is TextBox textBox)
            {
                ...
                if (textBox.Multiline)
                {
                    if (textBox.Parent is TableLayoutPanel)
                    {
                        ...
                        var customVScrollBar = new CustomVScrollBar(this, textBox, (int)textBox.Font.GetHeight(), GetTextBoxScrollBarMetrics, borderWidth);
            ...
            else if (control is ListBox)
            {
                ...
                {
                    listBox.KeyPress += CustomComboBox_KeyPress;
                    listBox.KeyDown += CustomComboBox_KeyDown;
                }
                else
                {
                ...
                if (borderWidth != -2)
                {
                    ...
                    {
                        var customVScrollBar = new CustomVScrollBar(this, listBox, listBox.ItemHeight, GetListBoxScrollBarMetrics, borderWidth);
                        customVScrollBar.CreateBrushes();
            ...
            else if (control is DataGridView dataGridView)
            {
                ...
                var customVScrollBar = new CustomVScrollBar(this, dataGridView, dataGridView.RowTemplate.Height, GetDataGridViewScrollBarMetrics, 0, vScrollBar, true);
                customVScrollBar.CreateBrushes();
                vScrollBar.Visible = false; //-V3080
        ...
        internal protected void initAndShow(Form ownerForm)
        {
            ...
            if (!IsPluginInitialized && this.GetType().FullName != "MusicBeePlugin.PluginSampleWindow")
            {
                MessageBox.Show(MbForm, PluginInitializationIsNotCompleted, string.Empty, MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
